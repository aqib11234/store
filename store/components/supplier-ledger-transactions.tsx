...
interface SupplierLedgerTransactionsProps {
  ledgerId: number
}

export function SupplierLedgerTransactions({ ledgerId }: SupplierLedgerTransactionsProps) {
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true)
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/supplier-ledgers/${ledgerId}/transactions/`)
  ...
  const getAmountColor = (type: string) => {
    // Debit transactions (increase our liability - we owe supplier more)
    if (['purchase', 'interest'].includes(type)) {
      return 'text-red-600'
    }
    // Credit transactions (decrease our liability - we owe supplier less)
